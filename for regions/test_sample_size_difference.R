#This script calculates and plots the effect of reducing the number of samples 
#when calculating the CCM measure.
#MUST RUN lump_csd_by_region.R FIRST
test_sample_nums <- function(sample.sizes=seq(10,100,by=10), time.window=2000, time.step=2000)
{
  #import libraries
  library(rEDM)
  library(snow)
  ...
  calc_pair_causality <- function(i, j, start.time, sample.size)
  {
    ...
    Ch2_xmap_Ch1 <- ccm(regional_EstCSD[start.time:end.time,], E = bestE_j, lib_column = j, first_column_time = TRUE,
      target_column = i, lib_sizes = 80, random_libs=TRUE, num_samples=sample.size)
    ...
    c(to=j, from=i, rho=max(0,ch2_map_1_mean$rho),
      start.time=start.time, time.window=time.window, sample.size=sample.size)
  }
  ...
  #only test for first 50 seconds
  start.times <- seq(time.step, 50000, by=time.step) - time.step
  #repeat the start times for each combination of 
  start.times.array <- rep(start.times, each=length(combinations[,2]))
  sample.sizes.array <- rep(sample.sizes, each=length(start.times.array))
  tick = proc.time() #start timing
  
  #calculate observed causality and put it in a dataframe
  pair.causality.data <- t(as.data.frame(clusterMap(cl, calc_pair_causality, combinations[,2], combinations[,1], start.times.array, sample.sizes.array)))
...
pairwise.causality.CSD.data <- calculate_pairwise_causality_CSD_full_experiment(time.window = 200, time.step=50)
