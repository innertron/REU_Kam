#this script calculates pairwise causality between brain regions using the raw signal data then
#stores the resulting data in a folder.
#MUST RUN lump_csd_by_region.R FIRST
calculate_pairwise_causality_full_experiment <- function(time.window=2000)
{
  #import libraries
  library(rEDM)
  library(snow)
  ...
  num.regions <- length(regional_neural_data)-1
  ...
  calc_pair_causality <- function(i, j, start.time)
  {
    ...
    Ch2_xmap_Ch1 <- ccm(regional_neural_data[start.time:end.time,], E = bestE_j, lib_column = j, first_column_time = TRUE,
...
dput(pairwise.causality.data, "~/pairwise.causality.regional.data.RData")