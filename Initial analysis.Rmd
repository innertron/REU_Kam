...
```{r}
hist(netw$strength, main="Distribution of cross map skill between region pairs \n during first second", xlab=expression(paste("cross map skill (", rho,")")))
```
...
for (i in 1:31)
{
  for (j in 1:31)
  {
    if (i != j)
    {
      ...
      rho_diff <- c(rho_diff, netw$strength[netw$from==i & netw$to==j] -
                        netw$strength[netw$from==j & netw$to==i])
    }
  }
}
hist(rho_diff, main = expression(paste("Distribution of difference of ", rho, 
                                       " between regions in each pair")),
